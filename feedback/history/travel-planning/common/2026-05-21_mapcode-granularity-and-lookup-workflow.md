...
**Step 0：確認目的地是否已知（前置條件）**

行程中若某一停靠點只有「料理ジャンル」或「地域」描述、**尚未有具體店名或設施名**（例如：「角館 稲庭うどん 昼食」「むつ市 活イカ料理」），必須先完成店家搜尋再進入 Step 1-5：

```
目的地名 = 已知（景點、旅館、具體店名）
  → 直接進入 Step 1

目的地名 = 未知（只有料理ジャンル・地域描述）
  → Step 0a：先搜尋候選店家
  → Step 0b：從候選中選定一家
  → 再進入 Step 1 查 mapcode
```

**Step 0a：搜尋候選店家**
- WebSearch：`[地域] [料理ジャンル] おすすめ ランチ 店 2025`
- 優先選擇：食べログ・Retty・じゃらん 等評論平台出現頻率高的店
- 考量因素：行程動線（前一個景點 → 餐廳 → 下一個景點）、步行圈內或需另行導航

**Step 0b：判斷是否需要獨立 mapcode**
```
餐廳距當日已有 mapcode 的景點 < 500m（步行圈內）
  → 不需要獨立行，備註「○○停車場から徒歩」即可

餐廳為當日獨立的移動目的地（需重新設定導航）
  → 必須補充獨立 mapcode 行
```
...
|------|----------------|------|
| チェーン店、道の駅 | 幾乎都有登錄 | Step 1-2 可取得 |
| 観光地エリアの有名店 | 多數有登錄 | Step 1-2 可取得 |
| 地元の食堂・小規模店 | **登錄率低** | Step 3（部落格）或 Step 4（最寄りバス停） |
| 武家屋敷通りなど観光地内の店 | **景點 mapcode で代替可** | 步行圈內不需獨立行 |
...
- ❌ 行程餐廳欄只有「料理ジャンル」描述時，直接跳入 Step 1 查 mapcode（目的地未定，無法查）
- ❌ 假設「餐廳在景點附近」就省略 mapcode；若需重新開車導航，仍需獨立行
...
- **餐廳/食堂欄位若無具體店名，必須先執行 Step 0（搜尋候選店家並確定目的地），再查 mapcode**；不得以「ジャンル不明」為由跳過整個餐廳欄位
