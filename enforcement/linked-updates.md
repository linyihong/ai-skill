...
| `enforcement/escalation-policy.md` | `enforcement/README.md`、`enforcement/dependency-reading.md`、`enforcement/failure-learning-system.md`、`runtime/runtime.db` activation/runtime guard tables、`metadata/rules/escalation-policy.yaml`、`metadata/rules/README.md`；若接入 runtime guard，另同步 `runtime/README.md`、guard-chain canonical document 與 `runtime.db`。 |
...
| 修改 plan / implementation 前需求拷問、pre-build interrogation 或 framework source-of-truth gate | `workflow/software-delivery/requirements/pre-build-interrogation.md`、`workflow/software-delivery/execution-flow.md`、`workflow/software-delivery/README.md`、`plans/README.md`、`governance/lifecycle/README.md`、`knowledge/runtime/routing-registry.yaml`、相關 validation scenarios。 |
...
| 修改 runtime canonical config 或 compiler 規則 | `runtime/runtime.db` 是 committed canonical config；不得另提交 `runtime/**/*.yaml` mirror。修改後必須執行 `ai-skill runtime compile` / `ai-skill runtime validate`，並確認 **`runtime.db` 包含在 commit 中**。驗證：SQLite `runtime_config_documents` 與 projection tables 均有對應 rows。 |
