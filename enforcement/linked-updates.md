...
| 修改 runtime canonical config 或 compiler 規則 | `runtime/runtime.db` 是 committed canonical config；不得另提交 `runtime/runtime.db canonical documents` mirror。修改後必須執行 `ai-skill runtime compile` / `ai-skill runtime validate`，並確認 **`runtime.db` 包含在 commit 中**。驗證：SQLite `runtime_config_documents` 與 projection tables 均有對應 rows。 |
