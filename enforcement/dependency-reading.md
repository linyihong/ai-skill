...
> **已升級為 SQLite-canonical runtime state machine**：快速路徑與 committed canonical config 都在 [`runtime/runtime.db`](../runtime/runtime.db)。完整 documents 存於 `runtime_config_documents`，transaction / phase / gate tables 是 projection；不要提交 `runtime/runtime.db canonical documents` mirror。
