...
> **已升級為 declarative state machine**：請參考 [`runtime/transactions/transaction-machine.yaml`](../runtime/transactions/transaction-machine.yaml)。

本節原有的 procedural writeback transaction 邏輯（7 個狀態、entry conditions、blocking gates、transition rules）已從 prose 規則升級為 declarative YAML state machine。Agent 應直接讀取 transaction-machine.yaml 作為 authoritative reference，而非依賴本節的 prose 摘要。
...
### 交易關閉條件（由 transaction-machine.yaml 定義）

Transaction 的完整關閉條件已由 [`runtime/transactions/transaction-machine.yaml`](../runtime/transactions/transaction-machine.yaml) 定義（狀態機的 `closed` 狀態 entry_conditions）。Agent 應直接查閱該檔案，而非依賴本節的 prose 摘要。
