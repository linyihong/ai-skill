...
> **已升級為 compiled runtime state machine**：快速路徑請查 [`runtime/runtime.db`](../runtime/runtime.db) 的 transaction / phase / gate tables；source 在 [`runtime/compiler/embedded_data.rb`](../runtime/compiler/embedded_data.rb)。

本節原有的 procedural writeback transaction 邏輯（7 個狀態、entry conditions、blocking gates、transition rules）已從 prose 規則升級為 compiled runtime state machine。Agent 應查詢 `runtime/runtime.db` 作為 authoritative runtime reference；需要修改定義時再編輯 `runtime/compiler/embedded_data.rb`。不要引用已移除的 `runtime/transactions/transaction-machine.yaml`。
...
Transaction 的完整關閉條件已由 `runtime/runtime.db` 的 transaction tables 定義（source：`runtime/compiler/embedded_data.rb`）。Agent 應直接查詢 SQLite，而非依賴本節的 prose 摘要。
...
state_based_enforcement:
  ...
  description: >
    ...
    Agent 應優先查詢 runtime/runtime.db；需要修改時編輯 runtime/compiler/embedded_data.rb。
