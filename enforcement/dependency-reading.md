...
> **已升級為 compiled runtime state machine**：快速路徑請查 [`runtime/runtime.db`](../runtime/runtime.db) 的 transaction / phase / gate tables；source 在 [`runtime/transactions/`](../runtime/transactions/)、[`runtime/phases/`](../runtime/phases/) 與 [`runtime/gates/`](../runtime/gates/) YAML。

本節原有的 procedural writeback transaction 邏輯（7 個狀態、entry conditions、blocking gates、transition rules）已從 prose 規則升級為 compiled runtime state machine。Agent 應查詢 `runtime/runtime.db` 作為 authoritative runtime reference；需要修改定義時再編輯 `runtime/transactions/transaction-machine.yaml` 或相關 runtime YAML，然後執行 `ai-skill runtime compile`。
...
5. **最終狀態確認**：最後一次 `git status --short --branch` 顯示 clean，且 branch 沒有 ahead/behind。

### 交易關閉條件（由 runtime tables 定義）

Transaction 的完整關閉條件已由 `runtime/runtime.db` 的 transaction tables 定義（source：`runtime/transactions/` YAML）。Agent 應直接查詢 SQLite，而非依賴本節的 prose 摘要。
...
state_based_enforcement:
  ...
  description: >
    ...
    Agent 應優先查詢 runtime/runtime.db；需要修改時編輯 runtime YAML source。
