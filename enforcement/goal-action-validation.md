...
本規則適用於本庫所有 shared rules、skills、templates、feedback lessons，以及由 skill 產出的專案文件。目的不是要求暴露模型內部推理，而是讓每個對外可見的工作單元都有清楚的目標、實際做了什麼、以及如何確認沒有偏離。
...
- **參考來源**：讀過的文件、程式碼、規格、證據、使用者提供的條件、既有 shared rule 或 skill 條文。
