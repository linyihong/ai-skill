...
- `feedback_history/` 與 `enforcement/failure-patterns/` 之間失去分工：前者記技巧，後者記 agent failure prevention。
- 後續 agent 只會看到新增技巧，不會看到漏觸發 feedback 的 root cause 與 validation gate。

## Required Agent Action

1. 停止繼續專案工作或新增更多 skill-local lessons。
2. 讀取 `enforcement/failure-learning-system.md`、`enforcement/feedback-lessons.md`、`enforcement/reusable-guidance-boundary.md`、`enforcement/linked-updates.md` 與 `enforcement/failure-patterns/README.md`。
3. 分類本次失效：通常是 `dependency-miss`（未讀 failure-learning dependency）加上 `validation-gap`（未驗證 durable prevention）。
4. 若問題可能跨 skill / project / agent 重演，新增或更新 `enforcement/failure-patterns/`。
...
- `enforcement/failure-patterns/` 有對應 pattern 或既有 pattern 被更新。
- Skill-local `feedback_history/` 只保存 technique / domain lesson，不承擔 cross-skill agent failure prevention。
- `enforcement/feedback-lessons.md` 或 affected skill rule 明確提醒：agent mistake 或 close-loop gap 需走 failure-learning system。
