...
| 內容 | 放置位置 |
| --- | --- |
| 可重用失敗模式、原因分析、決策規則、驗證流程、適用 / 不適用條件 | `shared-rules/` 或對應 skill 文件 |
...
3. **失效學習**：若該問題可能重複發生，依 [`failure-learning-system.md`](failure-learning-system.md) 分類失效模式，必要時新增或更新 `shared-rules/failure-patterns/` 或對應 skill 的 `feedback_history/`。
