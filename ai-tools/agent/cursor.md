...
- `ai-skill init-project --tools cursor` 產生的 project hook 必須包含 final-response close-out checks（目前使用 `afterAgentResponse` 與 `stop`），呼叫 repo-local `ai-skill hooks run stop`，在 final response 缺少 compact `Cognitive:` / full `### Cognitive Mode 報告`，或 hook payload 無法提供 assistant response text 時 fail closed / loop back。這是 chat/session 層防漏；格式與枚舉仍只在 canonical bootstrap sources。
