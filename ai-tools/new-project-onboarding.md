...
| **Cursor** | `.cursor/rules/ai-skill-bootstrap.mdc` + `.cursor/hooks.json` | thin alwaysApply pointer；session 啟動提示 bootstrap，final-response hooks 檢查 final Cognitive Mode 報告 |
...
- `afterAgentResponse` / `stop`：呼叫 repo-local `ai-skill hooks run stop`；若 final response 缺 compact `Cognitive:` / full `### Cognitive Mode 報告`，或 hook payload 無法提供 assistant response text，fail closed / loop back。Cognitive 格式與枚舉仍只在 canonical bootstrap sources，不複製到 Cursor rule。
