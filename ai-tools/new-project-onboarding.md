...
| **Cursor** | `.cursor/rules/ai-skill-bootstrap.mdc` + `.cursor/hooks.json` | thin alwaysApply pointer，session 啟動時載入 bootstrap canonical contract |
| **Claude Code** | `CLAUDE.md` + `.claude/settings.json` | thin bootstrap pointer + Go hook runner，載入 bootstrap canonical contract 並強制 per-turn Cognitive Mode / nested Git report |
...
| **GitHub Copilot** | `.github/copilot-instructions.md` + `.github/instructions/ai-skill.instructions.md` | compatibility thin pointer；Copilot 導流到 canonical source，hard enforcement 由 hooks / CI / runtime validate 負責 |
...
- `UserPromptSubmit`：注入 per-turn Cognitive Mode obligation，並掃描 project root 底下的 dirty nested Git repos。
...
### GitHub Copilot（`.github/copilot-instructions.md` + `.github/instructions/ai-skill.instructions.md`）

建立 Copilot project-wide custom instructions 與 scoped instructions，兩者都保持 thin pointer：
...
3. **Enforcement boundary**：Copilot instructions 只導流；若 Copilot 無法可靠強制 runtime gate，hard enforcement 仍交給 repository hooks、CI 與 `ai-skill runtime validate`。
...
| GitHub Copilot | `.github/copilot-instructions.md`、`.github/instructions/ai-skill.instructions.md` | [`agent/copilot.md`](agent/copilot.md) |
