...
| **Cursor** | `.cursor/rules/ai-skill-bootstrap.mdc` + `.cursor/hooks.json` | thin alwaysApply pointer；session 啟動提示 bootstrap，stop hook 以 `followup_message` loop back 檢查 final Cognitive Mode 報告 |
...
- `stop`：呼叫 repo-local `ai-skill hooks run stop`；若 final response 缺 compact `Cognitive:` / full `### Cognitive Mode 報告`，hook runner 輸出 `followup_message` 並 exit 0 讓 Cursor loop back。Cognitive 格式與枚舉仍只在 canonical bootstrap sources，不複製到 Cursor rule。
