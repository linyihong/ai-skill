...
| **Cursor** | `.cursor/rules/ai-skill-bootstrap.mdc` + `.cursor/hooks.json` | thin alwaysApply pointer；session 啟動提示 bootstrap，session stop 檢查 final Cognitive Mode 報告 |
...
- `stop`：呼叫 repo-local `ai-skill hooks run stop`；若 final response 缺 compact `Cognitive:` 或 full `### Cognitive Mode 報告`，fail closed / loop back。Cognitive 格式與枚舉仍只在 canonical bootstrap sources，不複製到 Cursor rule。
