...
| [`java-tsv-trim-split-trailing-empty.md`](java-tsv-trim-split-trailing-empty.md) | Java TSV `trim()` 在 `split("\t", -1)` 前破壞尾端空欄位 — `String.trim()` 會移除 tab 字元 (0x09)，導致 TSV 尾端空欄位被靜默丟棄 | `intelligence/engineering/analytical-reasoning/heuristics/`（原位置，已遷移） |
