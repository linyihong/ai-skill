...
4. **不重複 ADR**：Project memory 只保留架構決策的摘要與連結，完整決策放在 `memory/decision/` 或 `decisions/`。
...
- `shared-rules/failure-patterns/` 從 `memory/failure/` promotion 可執行規則。
...
- [`shared-rules/failure-learning-system.md`](../shared-rules/failure-learning-system.md) — 失效學習流程
