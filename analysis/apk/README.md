...
- 工具 skill 觸發文字；使用 `workflow/apk-analysis/execution-flow.md`。
...
| 主題 | 目前來源 | 提取狀態 |
| --- | --- | --- |
| 通用流量／執行路徑分流 | `../../skills/apk-analysis/WORKFLOW.md`（已刪除） | ✅ 已萃取至 `traffic-triage.md` |
| 工具選擇與失敗判讀 | `../../skills/apk-analysis/TOOLS.md`（已刪除） | ✅ 已萃取至 `tools-and-failures.md` |
| 媒體驗證工具 | `../../skills/apk-analysis/TOOLS.md`（已刪除） | ✅ 已萃取至 `tools-and-failures.md` |
| 自動化腳本安全邊界 | `../../skills/apk-analysis/TOOLS.md`（已刪除） | ✅ 已萃取至 `tools-and-failures.md` |
...
- 流量分流、工具選擇、失敗判讀、媒體驗證與安全邊界已從舊 `skills/apk-analysis/` 萃取至本目錄。
- 舊 `skills/apk-analysis/` 已全部遷移至新分層，本目錄為 canonical source。
- 當 technique 被萃取時，使用 `../../metadata/schema.md` 建立 metadata，並更新 `../../knowledge/indexes/README.md`。
- `workflows/` 是 HOW TO DO 執行知識的目錄。
