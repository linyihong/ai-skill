...
`analysis/apk/workflows/local-proxy-hook-flow.md` 是從 `skills/apk-analysis/techniques/local-proxy/` 拆解出的 **HOW TO DO** 操作流程。決策智慧（何時該懷疑 local proxy、如何區分導流 vs TLS 問題）請見 `intelligence/engineering/apk-analysis/heuristics/local-proxy-routing-diagnosis.md`。
