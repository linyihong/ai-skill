# Templates

Use these templates when turning app analysis, product intent, or review findings into reusable development documents.

| File | When to use |
| --- | --- |
| [`initial-development-docs.md`](initial-development-docs.md) | Starting from a product brief or unclear requirements; drafts Product Brief, Bounded Contexts, BDD, Domain Model, Architecture, API / Interface, Error Handling, Test Plan, and Implementation Slices. |
| [`hardening-note.md`](hardening-note.md) | Capturing a reusable app/API development guidance note after an analysis or design review lesson. |
| [`threat-model-lite.md`](threat-model-lite.md) | Running a quick feature review for trust boundaries, abuse cases, controls, validation, and residual risk. |

## Required Linked Updates

Template changes affect how agents draft project documents. Follow [`../../../shared-rules/linked-updates.md`](../../../shared-rules/linked-updates.md): when a template changes, update or verify `../DOCUMENTATION.md`, `../process/`, `../CHECKLIST.md`, and any workflow or skill entry that tells users to copy the template.

If no linked update is needed, state why in the change note or commit message.
