...
| `2026-05-07_171900-provider-read-negative-matrix.md` | candidate | Provider read negative matrix | Prefer disabled-by-default no-writeback provider-read overrides over final query-map mutation when classifying bad/empty opaque provider behavior. |
| `2026-05-11_125615-per-round-feedback-checkpoint.md` | promoted | Per-round feedback checkpoint | Every meaningful APK-analysis round should check whether new technique or failure feedback needs skill writeback before continuing project work. |
| `2026-05-08_135700-agent-goals-before-transcripts.md` | candidate | Agent goals before transcripts when resuming | Resume interrupted work from `.agent-goals` first, then confirm with transcripts, terminal output, and git state. |
| `2026-05-08_141430-identity-material-self-generation-audit.md` | candidate | Identity material self-generation audit | Live-facing SDK/client work must classify required identity material by whether it can be self-generated, how it is generated, or which authorized provider owns it. |
