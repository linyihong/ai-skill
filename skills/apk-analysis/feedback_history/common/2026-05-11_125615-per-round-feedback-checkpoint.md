...
# Extracted — See [`shared-rules/feedback-lessons.md`](../../../../shared-rules/feedback-lessons.md)
...
4. 對 `failure-learning needed` 讀 `failure-learning-system.md` 並判斷是否需要 `shared-rules/failure-patterns/`。
5. 不把具體 App 名稱、sample id、host、payload 或本機路徑寫入 reusable lesson。

#### Goal / Action / Validation

- Goal: 降低長對話 APK 分析中漏寫 skill feedback 或錯誤回饋的機率。
- Action: 在 `SKILL.md` 與 `shared-rules/feedback-lessons.md` 加入 per-round feedback checkpoint，並以本 lesson 記錄泛化規則。
...
- `shared-rules/feedback-lessons.md` 明確要求每輪回饋檢查。
...
- 已依 [`linked-updates.md`](../../../../shared-rules/linked-updates.md) 檢查：本 lesson 已 promoted 到 `SKILL.md` 與 shared feedback rule；`feedback_history/README.md` 需要新增索引列。
- 已依 [`reusable-guidance-boundary.md`](../../../../shared-rules/reusable-guidance-boundary.md) 檢查：正文只保留 generalized checkpoint，具體 project evidence 留在 project docs。
