...
| `http-api/2026-05-05_183400-ui-automation-operation-scripts-for-api-capture.md` | promoted | UI automation operation scripts for API capture | 輕量 UI 架構地圖可以為關鍵 flow 產生小型可重放操作腳本，搭配 pcap/MITM/Frida 時間窗穩定抓 API。 |
| `http-api/2026-05-05_183700-scrollable-clickable-screen-mapping.md` | promoted | Scrollable and clickable screen mapping | UI-to-API mapping should classify each screen's scrollable regions and clickable entry points before writing automation scripts. |
| `http-api/2026-05-05_184700-screen-reachability-operation-recipes.md` | promoted | Screen reachability operation recipes | UI architecture maps should document how to reach important screens so manual replay, automation scripts, and API capture share the same route. |
| `http-api/2026-05-05_184900-in-app-route-map-external-transitions.md` | promoted | In-app route map external transitions | UI route maps should only expand in-app pages; external transitions should be documented as boundaries, not treated as app screens. |
| `http-api/2026-05-05_195200-feature-reconstruction-handoff.md` | promoted | Feature reconstruction handoff | Preserve feature, behavior, domain, API, state/error, fixture, and unknown details so `app-development-guidance` can rebuild the functionality. |
| `http-api/2026-05-06_081400-auto-app-guidance-for-sdk-tools.md` | promoted | Auto app-development-guidance for SDK/tool outputs | Automatically apply `app-development-guidance` when APK analysis docs are used to build app tools, SDKs, clients, mocks, contract tests, or rebuilt features. |
| `http-api/2026-05-05_202400-page-analysis-requires-ui-map-artifact.md` | promoted | Page analysis requires a UI map artifact | 針對具名頁面做 APK/API 分析時，必須在專案 docs 產出或更新 page-level 輕量 UI 架構地圖，不能只散落在 API / 工具文件。 |
| `http-api/2026-05-05_205500-ui-behavior-backfill-finish-gate.md` | promoted | UI Behavior backfill as finish gate | APK analysis is not complete until observed UI behavior is backfilled into the project UI behavior/page-map docs or explicitly marked as a documented gap. |
