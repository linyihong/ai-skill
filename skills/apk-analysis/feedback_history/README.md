...
| `2026-05-01_171500-json-shape-before-query-shape.md` | validated | JSON Shape Before Query Shape | Schema-only response logging should classify JSON before query extraction, otherwise embedded URLs inside JSON can be mistaken for request query evidence. |
| `2026-05-01_171650-ui-architecture-map-from-screenshots.md` | promoted | UI architecture map from screenshots | 在可控制裝置時，先用去敏 screenshot、UI hierarchy 與操作時間窗建立 App 架構地圖，再把 API 對應回具體 tab、screen 與 action。 |
