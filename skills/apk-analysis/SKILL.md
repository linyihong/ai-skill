...
8. **Automatic skill feedback (agents)**:
   ...
   - At the end of every meaningful work round, before returning to long project work, before committing project-only evidence, and when the user says「繼續」to start another round, run a short feedback checkpoint in the conversation: "Did this round add a reusable technique, validation rule, replay/runner guard, hook option, error pattern, or close-loop gap?" If yes, pause project work and open the canonical Ai-skill writeback transaction; if no, it is enough to say the round produced only project-specific evidence or no generalized lesson.
...
**Agent checklist before ending an APK-analysis task or work round:** Did any **new generalized lesson** emerge, including from user suggestions, review feedback, new runner/hook guards, validation matrix rows, error classifications, or a missed close-loop? If yes → **`feedback_history/<category>/`** or **`feedback_history/common/`** has a new file (minimum); optional promotion to `TOOLS.md` / `WORKFLOW.md` / `DOCUMENTATION.md` / `techniques/<category>/` when justified. If the lesson came from agent mistake or incomplete feedback closure, also evaluate `shared-rules/failure-learning-system.md` and `shared-rules/failure-patterns/`. If nothing new → no forced entry, but answer must state why the idea stayed project-specific or unvalidated when the user explicitly asked about skill feedback.
