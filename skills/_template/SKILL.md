...
**共用政策：**先讀 [`shared-rules` 索引](../../shared-rules/README.md)、[`feedback-lessons.md`](../../shared-rules/feedback-lessons.md)、[`neutral-language.md`](../../shared-rules/neutral-language.md)、[`document-sizing.md`](../../shared-rules/document-sizing.md) 與 [`cross-skill-references.md`](../../shared-rules/cross-skill-references.md)。`feedback_history/` 內各 lesson 只引用、不重複貼全文；文件標題、檔名、slug、索引與摘要使用中性低爭議用語；文件變大時改成資料夾與 `README.md` 目錄拆分。若本 skill 需要引用另一個 skill 的規範、模板或交接產物，只寫觸發條件、交接 artifact 與 ownership boundary，不複製對方全文。
