...
| Cursor | `.cursor/hooks.json` | sessionStart bootstrap prompt + afterAgentResponse / stop close-out hooks；hooks 委派 repo-local `ai-skill hooks run stop`，缺 final Cognitive Mode 報告或無法取得 assistant response 時 fail closed / loop back |
