...
| `scripts/validate-runtime-sqlite-index.rb` | 驗證本機 SQLite runtime index integrity、row count、FTS、checksum、git ignore | 無 flags | 無寫入；失敗時 non-zero exit | Ruby、sqlite3 CLI、Git | `ai-skill runtime validate` | `wrapper first` | stale checksum fixture；missing DB fixture（Phase 3 已開始 `runtime validate` wrapper-first：dry-run planner、Ruby/sqlite3 missing dependency、UTF-8 env 固定；完整 stale / missing DB fixture 待補） |
