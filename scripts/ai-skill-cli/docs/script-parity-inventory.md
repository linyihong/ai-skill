...
| --- | --- | --- | --- | --- | --- | --- | --- |
| deleted `scripts/init-new-project.sh` | 建立新專案 tool bootstrap：`.roomodes`、`.cursor/rules/`、`.cursor/hooks.json`、`CLAUDE.md`、`.agent-goals/` | 目標專案路徑、`--dry-run`、`--force`、`--tools` | 寫入目標專案設定；dry-run 只列出計畫 | Go filesystem APIs | `ai-skill init-project` | `deleted` | fake home / target project fixture；既有檔案衝突 fixture；force overwrite fixture；selected tools fixture；no write outside target assertion |
| deleted `scripts/agent-goals.sh` | 管理 project-local `.agent-goals/`、locks、goal index、split / pause / complete | `--project`、`init/status/start/update/split/pause/complete/cleanup` | 寫入 `.agent-goals/`，更新 `.git/info/exclude`，建立 / 清理 lock | Go filesystem APIs、可選 Git repository metadata | `ai-skill goals` | `deleted` | fake project fixture；lock fixture；stale cleanup fixture；`complete` 缺 `--validated` 不刪除 assertion；index refresh assertion |
...
| `scripts/sync-cursor-bundle.sh` | 同步 Cursor bundle / mirror symlink，清理會造成循環的 symlink | 環境中的 `$HOME` 與 repo root | 寫入 `~/.cursor/bundles/`、`~/.cursor/shared-rules`、`~/.cursor/skills/`；可能移動既有非 symlink 目標 | POSIX shell、find、ln、檔案權限、Cursor 目錄慣例 | `ai-skill sync-cursor-bundle` | `deleted` | Deleted by user decision as legacy shell surface. Go command currently supports dry-run planning only; future write support must be implemented in Go with managed mirror write parity, fake home fixture, copy fallback / symlink policy, and unmanaged target safety. |
| deleted `scripts/ai-skill-close-loop.sh` | 檢查 lock、dirty owner group、plan closure、knowledge runtime validation，依 group commit，可選 push | `--commit`、`--push`、`AI_SKILL_SYNC_CURSOR_BUNDLE`、`AI_SKILL_LOCK_TTL_SECONDS` | dry-run 列群組；`--commit` 執行 `git add/commit`；`--push` 執行 `git push`；`AI_SKILL_SYNC_CURSOR_BUNDLE=1` 由 Go hook runner / sync command 管理 | Go CLI、Git | `ai-skill close-loop` | `deleted` | temp repo grouped commit fixture；private-path scan fixture；plan closure warning fixture；runtime validation failure blocks commit；readback / clean status assertion |
...
| `runtime/compiler/compiler-engine.rb` | 編譯 canonical prose source / structured YAML 到 `runtime/runtime.db` | `--check`、`--diff`、`--db` | 更新或檢查 `runtime/runtime.db`，可輸出 diff | Ruby、YAML parser、sqlite3 CLI、runtime source | `ai-skill runtime compile` | `deleted` | Replaced by Go-native source-to-DB compiler reading runtime YAML plus `runtime/runtime.db`; custom DB, generated surface, row-count, and stable snapshot tests covered. |
...
| `scripts/sync-runtime-yaml-from-embedded.rb` | 從 embedded runtime data 回寫 runtime YAML 與缺失 README | 無 flags | 寫入多個 `runtime/runtime.db canonical documents` 與部分 README；一方向同步 | Ruby、runtime compiler embedded data | source restoration completed | `deleted` | Runtime SQLite canonical documents restored; no embedded runtime source remains. |
...
| `scripts/git-hooks/pre-commit` | staged runtime source 變更時編譯 `runtime.db`；staged knowledge / validation 變更時跑 repo-local CLI validator | Git staged files | 可更新並 stage `runtime/runtime.db`；可阻斷 commit | Git hook adapter、repo-local `ai-skill` binary | `ai-skill hooks run pre-commit` + `ai-skill hooks install` | `hook adapter - logic to Go` | staged runtime fixture；compiler failure blocks commit；runtime.db staged assertion；knowledge validation path uses Go hook runner |
| `scripts/git-hooks/post-commit` | Reference-only 預設 no-op；`AI_SKILL_SYNC_CURSOR_BUNDLE=1` 時提示 Go mirror write mode 未啟用 | `AI_SKILL_SYNC_CURSOR_BUNDLE` | 不寫入；failure 不阻斷 commit | Git hook adapter、repo-local `ai-skill` binary | `ai-skill hooks run post-commit` + future Go mirror write mode | `hook adapter - logic to Go` | env-off no-op fixture；env-on delegates to Go and must not call deleted shell |
...
- `native target - delete this cycle` 是本輪必須完成或明確回報 blocker 的 deletion target；不得留下 shell 作長期入口。
