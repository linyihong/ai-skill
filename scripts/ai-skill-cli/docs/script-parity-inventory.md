...
| `scripts/ai-skill-close-loop.sh` | 檢查 lock、dirty owner group、plan closure、knowledge runtime validation，依 group commit，可選 push | `--commit`、`--push`、`AI_SKILL_SYNC_CURSOR_BUNDLE`、`AI_SKILL_LOCK_TTL_SECONDS` | dry-run 列群組；`--commit` 執行 `git add/commit`；`--push` 執行 `git push`；`AI_SKILL_SYNC_CURSOR_BUNDLE=1` 只提示 sync shell 已刪 | POSIX shell、Git、repo-local `ai-skill` binary | `ai-skill close-loop` | `legacy retained` | Go command 目前只支援 dry-run inspection，`--commit` / `--push` 明確 blocked；刪除前必須完成 commit/push/private-path scan、plan closure、readback 與 grouped commit parity。Shell 內不得新增新功能。 |
...
| `scripts/git-hooks/post-commit` | Reference-only 預設 no-op；`AI_SKILL_SYNC_CURSOR_BUNDLE=1` 時提示舊 shell sync 已刪 | `AI_SKILL_SYNC_CURSOR_BUNDLE` | 不寫入；failure 不阻斷 commit | Git hook、POSIX shell | `ai-skill hooks install` + future Go mirror write mode | `hook adapter` | env-off no-op fixture；env-on must not call deleted shell |
