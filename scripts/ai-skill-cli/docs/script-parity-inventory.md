...
| `scripts/generate-knowledge-runtime-report.rb` | 從 routing registry、summaries、graphs、refresh policy 產生 deterministic runtime report | `--write` | 可輸出或寫入 `knowledge/runtime/runtime-report.md` | Ruby、YAML parser | `ai-skill runtime refresh` | `wrapper first` | golden output fixture 已覆蓋 runtime report anchors；source path link check 待 refresh / validator parity |
