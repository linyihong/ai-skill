...
| `scripts/sync-runtime-yaml-from-embedded.rb` | 從 embedded runtime data 回寫 runtime YAML 與缺失 README | 無 flags | 寫入多個 runtime YAML mirror 與部分 README；一方向同步 | Ruby、runtime compiler embedded data | source restoration completed | `deleted` | Runtime SQLite canonical documents restored; no embedded runtime source remains. |
