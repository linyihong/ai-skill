...
| `scripts/generate-runtime-sqlite-index.rb` | 產生本機 SQLite / FTS runtime lookup cache | `--output` 或預設路徑 | 寫入 git-ignored `knowledge/runtime/sqlite/runtime-index.sqlite` 或指定 DB | Ruby、sqlite3 CLI、canonical knowledge files | `ai-skill runtime refresh` | `wrapper first` | SQLite index fixture 已覆蓋 temp DB row counts 與 FTS hit；Go-native prototype 已覆蓋 Ruby vs Go atoms / sources / edges / fts row counts、source checksum map 與 FTS hit counts；row-level diff / recursive feedback fixture 待補 |
