...
| `scripts/generate-model-checklists.rb` | 從 routing registry 產生 per-model context-loading checklist | `--write` | 可輸出或寫入 `knowledge/runtime/model-checklists.md` | Ruby、YAML parser | `ai-skill runtime refresh` | `wrapper first` | golden output fixture 已覆蓋 checklist anchors；profile checklist full snapshot parity 待補 |
