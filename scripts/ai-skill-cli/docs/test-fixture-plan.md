...
- Phase 3 已新增 `runtime-generator-parity` fixtures：`generate-model-context-report.rb` 與 `generate-model-checklists.rb` stdout 均與各自 Go-native builder byte-for-byte 一致；這些 fixtures 只建立替換前護欄，尚不改 production refresh path。
