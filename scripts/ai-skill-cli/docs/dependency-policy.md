...
| Shell adapters | 不可新增為長期入口；只能是 hook adapter、thin bootstrap wrapper，或尚未完成 write-mode parity 的 retained legacy surface | Runtime validate / refresh / compile / query 與 Roo setting adapter 已是 native default；`sync-cursor-bundle.sh` / `ai-skill-close-loop.sh` 仍待 Go write-mode parity |
