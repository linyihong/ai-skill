...
Mobile implementation is deferred and out of scope for this cross-platform desktop runtime plan. This document only records the support boundary so future work does not imply mobile native binary support.
...
| iOS | Out of scope; no arbitrary native binary support | iOS 不允許一般用途 executable persistence |
| Android | Out of scope for this plan | Termux 可行但路徑、Git、權限與背景任務仍需專門驗證 |
| Mobile control plane | Future direction only | 手機觸發桌面 / server runner 最符合安全與平台限制；需另開安全與授權計畫 |
...
若未來另開 mobile plan，可行方向：

| Option | Decision | Notes |
| --- | --- | --- |
| App-contained runtime | Future plan only | 需要 App 開發、檔案權限、Git/SQLite/runtime bundling、App Store policy 評估 |
| Browser/WASM | Future research only | 適合 inspect UI、state validation、replay UI；local repo access 是主要限制 |
| SSH / remote runner | Future plan only | iPhone 作 control plane，runtime 在 macOS/Linux/Windows runner |
...
## Required Follow-Up If Mobile Is Pursued Later
