...
| `ai-skill hooks run stop` | 執行 Stop / final-response hook：從 transcript 或 hook payload 檢查 last assistant message 含 Cognitive Mode block（compact 或 full table）；若 payload 沒有 assistant response text 則 fail closed；若有 dirty root / nested Git repos 也要求 `### Project Git Report`；缺少時 block（exit 2） | 否 | 否 | Cross-platform Go script runtime |
