...
| `runtime/compiler/compiler-engine.rb` | `ai-skill runtime compile` | 低 | 預設保持 wrapper。`runtime compile --native-compiler` 已提供 snapshot mode，從既有 `runtime/runtime.db` 產生指定 `--db` output；rollback 是不帶 flag 回 Ruby compiler。 | runtime source keyword、`--check` no-op、`runtime.db` generated surface assertion、Ruby vs native generated-surfaces / metadata snapshot parity；完整 runtime config row-count parity 待真正 port compiler。 |
