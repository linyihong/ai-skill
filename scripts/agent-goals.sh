...
content = [
    ...
    "## Goal Table",
    "",
    "| Priority | Status | Goal | Open Work / Decisions | Planning / Todo Links | Next Action | Updated |",
    "| --- | --- | --- | --- | --- | --- | --- |",
]
if rows:
    content.extend("| " + " | ".join(row) + " |" for row in rows)
else:
    content.append("| - | - | No active goals | - | - | - | - |")
content.extend([
    "",
    "## Notes",
    "",
...
cmd_split() {
  ...
  local -a start_args
  start_args=(--id "${id}" --title "${title}" --source "Child goal of ${parent}" --priority "${priority}" --next "${next}" --criteria "${criteria:-Complete child goal ${title}.}")
  local link
  for link in "${plan_links[@]+"${plan_links[@]}"}"; do
