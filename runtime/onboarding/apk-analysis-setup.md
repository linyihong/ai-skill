...
```text
enforcement/feedback-lessons.md                              # 檔名規則與 lesson 模板（全庫共用）
...
- 如果發現新技巧，請**主動**在 feedback/history/apk-analysis/<category>/ **新增 lesson 檔**（跨分類用 common/；格式見 enforcement/feedback-lessons.md；不要等使用者說「記得回饋」），且要讓人類也看得懂；可同步更新 workflow/apk-analysis/execution-flow.md、workflow/apk-analysis/artifact-gates.md、analysis/apk/workflows/ 或 intelligence/engineering/analytical-reasoning/ 如已驗證。
...
請依 enforcement/feedback-lessons.md，在同一輪對話於 feedback/history/apk-analysis/<category>/ 或 feedback/history/apk-analysis/common/ **新增一個** lesson 檔。
