...
Generated by [`compiler-engine.rb`](compiler/compiler-engine.rb) from runtime YAML sources and embedded runtime data in [`compiler/embedded_data.rb`](compiler/embedded_data.rb). Rebuilt on every commit that touches runtime sources. **Do not edit manually.**

Some legacy runtime YAML sources have been embedded into `compiler/embedded_data.rb` and no longer exist as standalone files. When the `Source` column below points to `compiler/embedded_data.rb`, edit the embedded source and recompile `runtime.db`; do not create a new YAML file with the old path unless a dedicated source restoration migration is planned.
