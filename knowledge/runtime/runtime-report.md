...
| --- | --- | --- |
| Routing registry | [`routing-registry.yaml`](routing-registry.yaml) | 49 records |
...
| `route.intelligence.requirements-cognition` | `intelligence/engineering/requirements/README.md` | `small` | `summary-first` | Requirement → behavior contract → acceptance criteria → validation target → execution artifact is traceable; no unconfirmed feature is written as requirement; high coverage is not treated as test effectiveness without targeted proof when risk is high. |
...
| `intelligence.requirements-cognition` | `candidate` | [`requirements-cognition.md`](../summaries/requirements-cognition.md) | BDD 被表達為 requirements cognition：處理 ambiguity、actor intent、behavior boundary、acceptance criteria、traceability、validation target 與 test effectiveness，而不是 Gherkin everywhere。 |
...
| `graph.requirements-cognition` | `intelligence/engineering/requirements/README.md` | `candidate` | 9 | [`requirements-cognition.yaml`](../graphs/requirements-cognition.yaml) |
