...
| `route.governance.executable-contract-boundary` | Primary: `governance/lifecycle/executable-contract-boundary.yaml`<br>Compression: `source-backed`<br>Required: `governance/lifecycle/executable-contract-boundary.md`<br>`governance/lifecycle/compiler-philosophy.md`<br>`runtime/README.md`<br>`runtime/runtime.db`<br>Validation: YAML contract inventory 已載入；runtime_projection.enabled contract 已由 compiler 投影到 runtime.db； 普通 metadata / graph / validation YAML 不會自動進 runtime。  |
