...
| `shared-rules/error-learning-system.md` 或 `shared-rules/error-patterns/` | `shared-rules/README.md`、`shared-rules/content-layering.md`、`shared-rules/linked-updates.md`、`shared-rules/reusable-guidance-boundary.md`、`shared-rules/feedback-lessons.md`、`shared-rules/goal-action-validation.md`、`shared-rules/dependency-reading.md`、相關 tool 文件與被補強的 shared rule / skill workflow。 |
...
[`error-learning-system.md`](error-learning-system.md) 管的是「已發現錯誤如何變成可重用防呆規則」。當使用者指出 agent 反覆犯錯、寫錯 source/mirror、漏讀依賴、漏做驗證、忘記 goal、混入專案細節或閉環不完整時：

- 先依本檔與相關 shared rule 補救當前 close-loop。
- 再用 `error-learning-system.md` 分類錯誤、選擇 promotion target，必要時新增或更新 `shared-rules/error-patterns/`。
- 若錯誤是 skill-specific，將 lesson 放到該 skill 的 `feedback_history/`；若是 cross-skill 或全庫行為，放到 `shared-rules/error-patterns/` 或對應 shared rule。
- 不可把 project incident 的 raw evidence、私人路徑、host、token 或一次性細節寫進 error pattern；先依 `reusable-guidance-boundary.md` 抽象化。
