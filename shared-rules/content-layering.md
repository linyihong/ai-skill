...
| 可重用的**單一技巧、lesson 全文** | 新分層：`feedback/extraction/<domain>-index.md`（索引）；舊結構保留：`skills/<skill-name>/feedback_history/`（向後相容，僅限尚未遷移的 skill）。成熟 lesson 可 promotion 到 `intelligence/<domain>/` 或 `workflow/<domain>/` |
