...
| 可重用的**單一技巧、lesson 全文** | 新分層：`feedback/extraction/<domain>-index.md`（索引）；舊結構保留：`skills/<skill-name>/feedback_history/`（向後相容）。成熟 lesson 可 promotion 到 `intelligence/<domain>/` 或 `workflow/<domain>/` |
| 某 skill 的**工具策略 adapter**（同一 skill 在某 AI 工具上的執行差異） | 新分層：`tools/adapters/<tool>.md`；舊結構保留：`skills/<skill-name>/tool-adapters/<tool>.md`（向後相容）。只寫該工具差異並連回核心 workflow，工具全域設定仍放 `ai-tools/<tool>.md` |
