...
2. **啟動流程：**每次 session 啟動時，先讀 [`CORE_BOOTSTRAP.md`](../CORE_BOOTSTRAP.md)（3 條核心規則，~800 tokens），再讀此 README 的索引與 **[Runtime Activation Model](#runtime-activation-model)**。不要一次把所有 shared rules 當作已讀。若規則看似衝突，先依 [rule-weight.md](rule-weight.md) 判斷權重。
