...
| [`active/2026-06-10-1718-software-delivery-governance-invariants.md`](active/2026-06-10-1718-software-delivery-governance-invariants.md) | draft | Software Delivery Governance Invariants：把 browser capability fallback、side-effect counting、deploy env drift、nested repo dirty state、interrupted deploy、browser smoke，以及 workflow activation / escalation miss 等 incident feedback 提煉成 reusable workflow invariants；候選包括 `workflow-activation`、`workflow-escalation`、`explicit-root-cause-hypothesis`、`change-boundary-detection`、`runtime-capability-validation`、`authority-coupled-side-effects`、`configuration-readback-validation`、`operational-transaction-closure`、`journey-level-post-deploy-validation`，先規劃 placement / scenarios，不新增 runtime gate。 |
