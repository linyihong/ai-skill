...
| [`active/2026-06-16-1131-evidence-candidate-system.md`](active/2026-06-16-1131-evidence-candidate-system.md) | draft | Evidence Candidate System（跨 plan 候選證據層）：解「人工記不住哪個案例該回流到哪個 plan」的**索引問題**。被動 Detection + 人工 Capture，**不做 Enforcement / Promotion**。成熟度階梯 Detection/Capture/Enforcement/Promotion；plan 各自宣告 evidence-rule / acceptance-gate / notify-rule（notify = 可以 review ≠ 已升級）；memory 只存 pointer 不存 evidence。**不接入 runtime**；Phase 2 accumulation runtime deferred until ≥20 candidate / >50% accept。本 plan 自己是 N=1，不抽共用 primitive（走 governance-pattern N≥5 紀律）。 |
