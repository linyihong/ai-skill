...
| [`active/2026-06-10-1718-software-delivery-governance-invariants.md`](active/2026-06-10-1718-software-delivery-governance-invariants.md) | draft | Software Delivery Governance Invariants：把 browser capability fallback、side-effect counting、deploy env drift、nested repo dirty state、interrupted deploy、browser smoke 等 incident feedback 提煉成 reusable validation-family invariants；候選包括 `runtime-capability-validation`、`authority-coupled-side-effects`、`configuration-readback-validation`、`ownership-boundary-validation`、`operational-transaction-closure`、`journey-level-post-deploy-validation`。`workflow-activation` / `workflow-escalation` 暫定位為 routing pressure，`explicit-root-cause-hypothesis` 暫定位為 evidence acquisition discipline；Phase 0 ready，聚焦 routing pressure、evidence acquisition、`ownership-boundary-validation` temporary working name 是否拆成 task scope / ownership awareness、以及 authority/readback/closure/journey 是否真的共享 validation structure。`claim_validation` 僅保留為 hypothesis，不新增 runtime gate。 |
