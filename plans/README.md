...
| [`active/2026-06-10-1718-software-delivery-governance-invariants.md`](active/2026-06-10-1718-software-delivery-governance-invariants.md) | in-progress | Software Delivery Governance Invariants：把 browser capability fallback、side-effect counting、deploy env drift、nested repo dirty state、interrupted deploy、browser smoke 等 incident feedback 提煉成 reusable validation-family invariants。Phase 0–4 已完成：activation/escalation = routing pressure；root-cause hypothesis = evidence acquisition discipline；configuration readback = State Visibility / Evidence Chain-backed workflow evidence shape；`ownership-boundary-validation` 拆成 `task-scope-validation` + `ownership-awareness`；authority side effects = artifact / validation evidence shape；新增 10 個 scenario-first validation YAML；workflow docs 已加入 evidence shapes；glossary 新增 `authority_coupled_side_effects`、`configuration_readback_validation`、`task_scope_validation`、`ownership_awareness` candidate terms。`runtime-capability-validation` 與 `operational-transaction-closure` 暫不進 glossary；`claim_validation` 保持 hypothesis；未新增 runtime gate。下一步是 Phase 5 runtime refresh + closure。 |
