...
| [`active/2026-06-10-1718-software-delivery-governance-invariants.md`](active/2026-06-10-1718-software-delivery-governance-invariants.md) | in-progress | Software Delivery Governance Invariants：把 browser capability fallback、side-effect counting、deploy env drift、nested repo dirty state、interrupted deploy、browser smoke 等 incident feedback 提煉成 reusable validation-family invariants。Phase 0–2 已完成：activation/escalation = routing pressure；root-cause hypothesis = evidence acquisition discipline；configuration readback = State Visibility / Evidence Chain-backed workflow evidence shape；`ownership-boundary-validation` 拆成 `task-scope-validation` + `ownership-awareness`；authority side effects = artifact / validation evidence shape；operational transaction closure 暫保留為 shared execution reasoning candidate；`claim_validation` 保持 hypothesis；新增 10 個 scenario-first validation YAML，全部標記 fail-by-absence，尚不新增 runtime gate。下一步是 Phase 3 workflow documentation update。 |
