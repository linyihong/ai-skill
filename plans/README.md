...
| [`active/2026-06-10-1718-software-delivery-governance-invariants.md`](active/2026-06-10-1718-software-delivery-governance-invariants.md) | in-progress | Software Delivery Governance Invariants：把 browser capability fallback、side-effect counting、deploy env drift、nested repo dirty state、interrupted deploy、browser smoke 等 incident feedback 提煉成 reusable validation-family invariants；候選包括 `runtime-capability-validation`、`authority-coupled-side-effects`、`configuration-readback-validation`、`ownership-boundary-validation`、`operational-transaction-closure`、`journey-level-post-deploy-validation`。Phase 0 已開始：`workflow-activation` / `workflow-escalation` resolved as routing pressure，`explicit-root-cause-hypothesis` resolved as evidence acquisition discipline，`configuration-readback-validation` resolved as State Visibility / Evidence Chain-backed workflow evidence shape，`claim_validation` 保持 hypothesis；task scope vs ownership、authority side-effect shape、runtime capability breadth、operational transaction placement 與 validation scenarios 進 Phase 1/2。 |
