...
| [`active/2026-05-21-0834-cross-platform-go-script-runtime.md`](active/2026-05-21-0834-cross-platform-go-script-runtime.md) | 🚧 in-progress | Cross-Platform Go Script Runtime — 將 scripts 層升級為 Windows、macOS、Linux 可執行的單一 Go binary / runtime toolchain，優先內建 pure Go SQLite（`modernc.org/sqlite` candidate）、YAML、JSON 與 runtime logic；desktop Git 作為 external dependency，缺 Git 時由 `doctor` / linked-update flow 阻斷並提示安裝；iOS 以 App sandbox、Browser/WASM、SSH remote runner / control plane 評估 |
