...
**Status**：completed（2026-06-04）— sub-plan 詳見 [`01-frontmatter-schema.md`](01-frontmatter-schema.md)。本主計畫驗證要點：schema 文件化 + ≥ 3 個 fixture（main / sub / spike）。**全部達成**。
...
- [x] `governance/lifecycle/plan-tree-hierarchy.md` rule（Phase 5 status 升 validated）

**Phase 1 close-out（2026-06-04）**：
- [x] Sub-plan acceptance criteria 全 checked（Phase 2 已 completed 並重用 fixtures 作 testdata）
- [x] Phase 5 收尾連動 `plans/README.md` §Plan Tree Hierarchy、glossary 5 terms 註冊
...
- [x] 更新 `plans/README.md`：新增 §Plan Tree Hierarchy 章節，連結本 plan + 治理規則 + glossary + 5 validators 表
- [x] 更新 `governance/lifecycle/plan-tree-hierarchy.md` rule（Phase 1 已建；Phase 5 status draft → validated，validator 表更新為「已落地」）
- [x] 註冊 glossary terms 到 `knowledge/glossary/ai-skill.md`（`plan_kind` / `parent` / `plan_tree` / `required_for_completion` / `sub_plan_reason`，owner-layer `validation-governance`，`ai-skill glossary validate` 對新 entry 0 violation）
- [x] 寫 failure pattern `enforcement/failure-patterns/plan-tree-flat-ambiguity.md` + README 索引列
- [x] 5 platform binaries rebuild + push — **N/A**：Phase 5 純 docs/governance/glossary/plan markdown，無 Go source 改動；既有 binaries 已對齊最後一次 Go 變更（`cb713eb` rebuild from `22fcf4e`）。無 rebuild 必要。
- [x] 本 plan 整 folder 搬 `plans/archived/`（本 close-out commit）
...
- [x] Phase 0 preflight 全部完成
- [x] 4 個 sub-plan 全部 `status: completed`（隨主計畫 folder 一起 archive）
- [x] Phase 5 收尾項目全 checked
- [x] `governance/lifecycle/plan-tree-hierarchy.md` 落地（status validated）
- [x] 5 個 validator 進 `hooks.go` registry（4 block + 1 warning），unit test pass（Phase 2 `6fd1744`）
- [x] `ai-skill plans tree` CLI 可用（Phase 3 `22fcf4e`）
- [x] 至少 1 組真實 parent-child plan 用新格式跑完（registry cluster + 本 plan 自身 dogfood，Phase 4 `41d8d57`）
- [x] Failure pattern 文件化（`enforcement/failure-patterns/plan-tree-flat-ambiguity.md`）
- [x] Glossary terms 註冊（5 terms，owner-layer `validation-governance`）
- [x] 5 platform binaries rebuilt + pushed — N/A（Phase 5 無 Go 改動，binaries 已對齊 `cb713eb`）
- [x] 本 plan 整 folder 搬 `plans/archived/2026-06-02-1200-plan-tree-hierarchy-governance/`
...
**已於 Phase 5（2026-06-04）註冊**：5 terms 以 snake_case 進 `knowledge/glossary/ai-skill.md`，owner-layer `validation-governance`，status `canonical`，`related_to` 對稱關係以 `plan_tree` 為 hub。`ai-skill glossary validate` 對新 entry 0 violation（既有 `cognitive_slice` owner-layer 違規為 pre-existing，不在本 plan 範圍）。
