...
| Pri | # | 問題 | 處置 |
|---|---|---|---|
| P0 | T1 | F19 第 3 次被指出應為 `pending_implementation`（coverage_evidence schema 已存在，只是 executor 未實作 = 已知 implementation gap，不是 behavioral） | (a) **改採方案 A**：開 stub child plan `plans/active/2026-06-01-0100-validation-scenario-governance-executor.md`（內容僅 Phase 0/1 outline + 預估 scope，不立即實作）；(b) F19 改 `coverage=pending_implementation`，`child_plan` 指向此 stub；(c) 撤回 round-3 對 F19 的 sunset_decision 段落 | <!-- archival-provenance --> <!-- UPDATE 2026-06-12: F19 validation_scenario_governance promoted pending_implementation → mechanical; child plan completed and archived at plans/archived/2026-06-01-0100-validation-scenario-governance-executor.md -->
