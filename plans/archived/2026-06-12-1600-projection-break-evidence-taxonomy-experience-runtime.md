...
**Delivery**：✅ completed（commits `6d029f3`–`8428c00`）
**Plan archive**：✅ archived 2026-06-12
...
**Downstream pilot plan**（專案 repo `docs/plans/`，**不**複製進 Ai-skill — 見 [`reusable-guidance-boundary.md`](../../enforcement/reusable-guidance-boundary.md)）：`2026-06-12-1600-short-drama-player-validation-gaps.md` — 第一個消費本 plan 的 evidence taxonomy + gate tokens + cross-cutting alignment。
...
### ADR Promotion Criteria（**ADR not promoted** — criteria audit）

| Criterion | Status |
|---|---|
| foundational + cross-session + cross-project + expensive-to-reverse + explains-why | not met — OQ-1 still-open; slice promotion needs editor/onboarding |
| 至少一個 downstream project 用 evidence tokens 完成 real task closure | **met** — Vidoe-Test player pilot |
| Open Questions 全解 | not met — OQ-1 still-open; OQ-4 deferred |
| 沒有更輕的 promotion target 適用 | doc-only landing sufficient for pilot |
| validation scenarios 與 project gate 真實引用 evidence types | **met** — scenarios + downstream `claim_registry` |

Follow-up ADR evaluation deferred until OQ-1 runtime registry decision and second player-class pilot.
...
- [x] Glossary candidate terms 已註冊 — `knowledge/glossary/ai-skill.md`（6 entries, `status: candidate`）
- [x] 執行 Plan Completion Closure（validator + linked-updates + archive）— 2026-06-12
...
## Stakeholder 同意項目（2026-06-12 review + downstream pilot closure）

- [x] OQ-5 reject inheritance 可接受
- [x] `temporal_behavior` 取代 `timing_gate` 可接受
- [x] evidence_type / collection_method / artifact_shape 三分可接受
- [x] `browser_review` 降級為 activity label 可接受
- [x] experience-runtime 先 cross-cutting、三案例後再升 slice 可接受
- [x] Failure→Authority→Evolution 四步可接受
- [x] Phase 1–2 doc-only、不接 runtime gate 可接受

## Follow-up（out of scope for this plan）

| Item | Owner | Trigger |
|---|---|---|
| OQ-1 `evidence:*` in runtime registry vs project-local | new plan | 2nd player-class pilot + runtime projection follow-up |
| experience-runtime slice promotion | new plan | editor + onboarding pilots |
| Glossary `candidate` → `canonical` promotion | glossary plan / ADR | cross-project pressure + OQ-1 resolved |
