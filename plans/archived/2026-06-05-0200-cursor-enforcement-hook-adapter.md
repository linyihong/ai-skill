...
#### Bugfix Addendum — Cursor self-lockout + Plan→Build stop false positive（2026-06-05）

1. **Cursor preToolUse self-lockout（confirmed）**：Cursor 實際工具名可能是
   `ReadFile` / `functions.ReadFile` / `Glob` / `functions.Glob` / `rg` /
   `functions.rg` / `SemanticSearch` / `functions.SemanticSearch`，但 Phase 2
   read-allowlist 只涵蓋早期 static payload 名稱（`read_file` / `glob_file_search`
   等）。結果 bootstrap gate 要求先讀 canonical files，卻把實際 ReadFile 也擋住，
   形成 ReadFile / Glob / Shell / ApplyPatch 全部不可用的自鎖。
   **修法**：`preToolUseReadAllowed(hostCursor, tool)` 改為大小寫正規化後放行
   Cursor native 與 `functions.*` read/search 工具；非 read 工具（shell / patch /
   write / MCP mutation）仍受 bootstrap 與 workflow gate 約束。另同步
   `transcriptHasRequiredBootstrapReads`，讓 Cursor transcript 中
   `ReadFile` / `functions.ReadFile` 的 `input.path` 能滿足 bootstrap read-log
   requirement。
2. **Cursor stop Plan→Build handoff false positive（confirmed）**：Cursor mode
   transition / Plan→Build handoff 會觸發 `stop`，但該事件不是 final
   user-facing close-out；舊 guard 未涵蓋 Build mode，且沒有 assistant final text
   時仍會回 `followup_message`，造成錯誤要求 Cognitive Mode block。
   **修法**：Cursor stop 缺少 assistant final text 時 fail-open（不產生
   `followup_message`），並把 `build`/Plan→Build 模式切換文字加入
   `isCursorModeSwitchStatus`。一般 final response 缺 Cognitive 仍維持 followup。
...
- [x] Unit：Cursor actual read/search tool allowlist（`ReadFile` /
      `functions.ReadFile` 等）不要求 bootstrap receipt；Cursor ReadFile read-log
      可驗證 bootstrap canonical reads。
- [x] Unit：response 格式（native `{permission:deny}` / allow 空 stdout）—— `TestRenderCursorPreToolUseDecision_*`。
- [x] Unit：Cursor stop mode handoff / Plan→Build / no assistant final text 不回
      `followup_message`；一般 final response 缺 Cognitive 仍回 followup。
- [x] e2e：模擬 Cursor `preToolUse` payload 餵 `runPreToolUseHook` ——
      locked route + primary_source 未讀 → deny；已讀 → allow；miss → fail-open
      （commit d02e4a5，`TestRunPreToolUseHookCursorWorkflowGate_{LockedUnreadDenies,
      LockedAndReadAllows,MissAllows}`，全綠）。covers Claude e2e 等價案 — host
      detection / bootstrap read-log Cursor compat / detector / Cursor render 串
      一條 path 驗證。
- [x] 重建 5 平台 binary（commit d2838ef，BUILDINFO source_commit=d02e4a5，
      `TestRepoLocalBinariesMatchChecksumsAndCurrentSource` 綠）。

### Phase 5 — Docs + close-out

- [x] `command-contract.md`、`cursor.md`、`ai-tools/README.md` 同步（2026-06-05
      archive 收尾）：`command-contract.md` `hooks run pre-tool-use` row 改寫為
      host-aware（Claude + Cursor），新增 Cursor hooks 條目於 inputs；
      `ai-tools/README.md` agent table 加註 Cursor preToolUse 已是 mechanical
      gate；`cursor.md` 已於 Phase 3 完整描述 mechanical gate（無新增）。
- [x] failure-pattern `pretooluse-block-wrong-exit-code` 已存在
      ([`enforcement/failure-patterns/pretooluse-block-wrong-exit-code.md`](../../enforcement/failure-patterns/pretooluse-block-wrong-exit-code.md)，
      status=validated；涵蓋 Claude PreToolUse + Stop 兩條 exit-30 leak path
      與 Cursor `{permission:deny}` parity）。
- [x] enforcement-registry：**併入既有 `bootstrap_integrity` + `workflow_activation`**，
      不新增 `cursor_enforcement_parity` rule_class。理由：兩個 rule_class
      已綁定同一個 `runPreToolUseHook` / `finishPreToolUse` symbol，host
      dispatch 透過 `detectPreToolUseHost(payload)` + `renderPreToolUseDecision`
      在 executor 內部完成；新增獨立 class 會與既有 binding 重複。已在
      `bootstrap_integrity.executors[runPreToolUseHook].notes` 與
      `workflow_activation.executors[finishPreToolUse].notes` 補上 host-aware
      註記（Claude `permissionDecision:deny` / Cursor `{permission:deny}`）。
- [x] close-loop：git clean / push / readback（本 commit + plan archive commit
      完成後驗證）。
...
- [x] Cursor preToolUse payload 解析正確（unit）
- [x] Cursor actual `ReadFile` / `functions.ReadFile` preToolUse payload 放行，不要求 bootstrap receipt（unit）
- [x] Cursor stop Plan→Build / mode transition / no-assistant-final-text 不產生 followup；一般 final 缺 Cognitive 仍 followup（unit）
- [x] block 在 Cursor 真正 deny、allow 真正放行（e2e；`TestRunPreToolUseHookCursorWorkflowGate_{LockedUnreadDenies,LockedAndReadAllows}` 全綠）
- [x] advisory/miss 在 Cursor fail-open 不誤殺（e2e；`TestRunPreToolUseHookCursorWorkflowGate_MissAllows` 綠）
- [x] Claude 端 enforcement 仍正常（回歸；exit-code hotfix 後 `TestRenderClaudePreToolUseDecision_*`
      + `TestPreToolUseHookBlocksReceiptWithoutReads` 全綠，host=Claude path 通過 `go test ./...`）
...
- [x] close-loop：commit / push / readback / clean（2026-06-05 archive turn 收尾）
