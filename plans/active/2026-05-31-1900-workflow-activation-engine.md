...
- [x] Phase 1 schema 變更 backward compat（spec 層）— spec 僅新增 header block，未動 route record；accepted_format ≠ canonical_format 政策已寫入（user review 2026-06-04）
- [x] **Phase 1.5 normalizer validated（executor 層，user review 2026-06-04 要求拆出）** — `normalizeRouteTriggers` 已實作並測試：`TestNormalizeRouteTriggers_LegacyEqualsCanonical`（legacy ≡ canonical 正規化結果 byte-identical）+ `_CanonicalIdempotent`（canonical→canonical 穩定）+ `_MergesBothForms`。**「Rule Exists, Executor Missing」缺口已閉**：backward-compat 不再只是 spec 宣稱，runtime 已驗證。Normalizing ≠ Detecting，獨立成立
