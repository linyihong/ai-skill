...
|----|----------|---------------------|--------|-----------------|---------------------|
| Q1 | 外部 repo 跑 plan validators 的最薄強制機制（validator engine 被哪些 consumer 呼叫）？ | 01 | **ready-to-close（package assembled 2026-06-25；待 review 正式關）** | (1) equivalence proven **AND** (2) removal-independence proven **AND** (3) ≥1 consumer replaced without engine change — **三條全達成** | 01 §Phase 3.3：3.3a manual≡hook + 3.3b hook≡CI（+asymmetric F.3）+ 3.3c（R.1 removal-independence / R.2 contract-fingerprint stable / R.3 applicability / 真 replace CLI↔direct）。close package 見 01 §3.3c 表；本輪未自行 close，待 review |
