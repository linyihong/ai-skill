...
| Q3 | schema / 版本相容策略（pin 哪個 binary、schema version 怎麼宣告與演進）？ | 01 | open | `plan_schema` version 宣告 + 跨版本 acceptance pass | <commit + cross-version evidence>；**loader 需求發現（2026-06-24）**：外部 plan 以 `schema_version: "1"`（YAML 含引號）宣告，真實 loader 須 strip quote（`"1"`↔`1`）才能對 `currentSchemaVersion` 正確比對，否則誤拒 — 見 01 §Phase 3 canonical sample caveat |
...
| Q8 | **external schema compatibility boundary**：For repositories using non-canonical plan metadata, should compatibility be enforced by **adoption**, **normalization**, or **explicit unsupported declaration**?（刻意保留第三條 explicit-unsupported；不預設 mapping） | Phase 3（**deferred-to-phase-3**，不掛 01 current phases） | deferred-to-phase-3 | 待 Phase 3 跑真實外部 repo 後，依證據三選一 | pressure sample（**measured 2026-06-24**）：01 §Phase 3 External Pressure Samples。Vidoe-Test 13/13 解析、2/4 rules exercised、**2 false-positive**（`parent:` 持 path、engine 期待 id = **semantic mismatch**，不再只是 absence）。**boundary finding：portable ≠ schema-agnostic**。決定仍 deferred；不觸發 plan_profile reopen |
