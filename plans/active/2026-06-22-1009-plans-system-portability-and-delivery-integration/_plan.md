...
|----|----------|---------------------|--------|-----------------|---------------------|
| Q1 | 外部 repo 跑 plan validators 的最薄強制機制（validator engine 被哪些 consumer 呼叫）？ | 01 | open（盤點完成；3.3 待 impl） | **收緊（2026-06-25）**：(1) equivalence proven（per §Phase 3.3 Equivalence Matrix：RuleID/Blocking/opt-out/discovery-scope MUST equal）**AND** (2) removal-independence proven **AND** (3) **≥1 consumer replaced without engine change**（replaceable 是 Phase 3 起始契約） | 前置證據：01 §Phase 0.1 survey + 閉環 hook≡engine≡CLI（Phase 2.4）；待 3.3 impl 補 replaceability evidence + commit |
