...
- 若 projection 是非 canonical lookup cache，可放在 `knowledge/runtime/sqlite/` 或整合既有 runtime index；若成為 runtime-owned canonical config，必須走 `runtime/runtime.db` canonical boundary。
...
| 欄位 | 內容 |
| --- | --- |
| Runtime owner | Phase 1-4：Markdown canonical source + manual workflow reads；Phase 5：framework-approved semantic projection + runtime reports；Phase 5+ 若需要 blocking gate，再新增 owner-layer executable contract。 |
...
8. Semantic projection 應放在 dedicated `knowledge/runtime/sqlite/glossary.sqlite`、整合進現有 runtime index generator，還是提升為 runtime-owned projection table？
...
- [ ] 建立或規劃 framework-approved semantic projection 位置，至少支援 `glossary_terms`、`glossary_relations`、`glossary_usage`。
