...
>
> **EL-3A — 為什麼仍不開 candidate（admission order，不是 cross-repo 問題）**：admission 順序 = **Step 1 sample 可描述嗎 → Step 2 屬哪個 family → Step 3 該 family 收不收 cross-repo**。現在卡在 **Step 1**：`projection absent` → 連 `new_6step_sample` vs `nonfitting_sample` 都還不能確認，**family 未定** → 不標 criterion、不開 candidate、**不碰 cross-repo scope**（Step 3 還沒到）。EL-3（delivery lens）與 EL-3A（system lens）**並存**，後 lens 不覆寫前 lens。
>
