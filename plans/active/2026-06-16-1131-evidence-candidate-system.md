...
- [x] Phase 1C：stateless **assembler** scanner v0（`scanner-v0.py`）產生候選（≠ 自動接受；criteria 須源於 scanner 外）
...
**形式**：standalone stateless 腳本（[`governance/evidence-candidates/scanner-v0.py`](../../governance/evidence-candidates/scanner-v0.py)）。
**不接 ai-skill CLI、不接 hook、不接 runtime**（Option B git-diff / Option C CLI 皆 reject —— 會偷偷依賴
workflow / 讓 `accepted > scanner` 難以證明）。
...
完成物：
- [x] `scanner-v0.py`（assembler；stdlib only，不依賴 ai-skill CLI）
...
#### scanner v0 測試（2026-06-16，皆通過）
| # | 測試 | 結果 |
|---|---|---|
| T1 | 合法 emit（actor: human） | `EMIT C-43f95f0b → inbox`（gitignored）|
