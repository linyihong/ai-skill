...
每個 plan 自己宣告三件事（不是 System 寫死）：

```text
plan/
 ├─ evidence-rule.md     # 什麼 artifact 算我的候選證據（machine-readable criteria）
 ├─ acceptance-gate.md   # 達什麼條件算成熟
 └─ notify-rule.md       # 達條件時通知「可以 review」（不是「已升級」）
```
...
# governance-pattern — 達 N≥5 + falsification 提醒「可以考慮 review 是否 promotion」
collect: true
notify:
  sample_count >= 5
  falsification >= 1

# interaction-hazard — pilot 完成 + criteria 達標時提醒 Phase D gate 可評估
collect: true
notify:
  pilot_complete == true
  criteria_pass >= 6
...
- **D. Detection + Candidate Queue（被動發現 + 人工 accept），plan 自帶 evidence-rule /
  acceptance-gate / notify-rule**：**accept**。停在成熟度階梯的 Detection + Capture，不進
...
governance-pattern plan 的紀律。現在升 ADR = 自己違反自己。schema（confidence 要不要數字、store
committed 與否、scanner 觸發模型）也未驗證。
...
- [ ] 至少 2 個 plan 真的用 evidence-rule / notify-rule 各自判定（證明非 economics-only 特例）
- [ ] confidence-number Open Question 已解（見 §Open Questions）
...
#### 風險
- **confidence 數字可能是被夾帶的結論**（見 Open Question Q1）——機器對「屬於哪個 plan」做判定，
  即使人工 accept，也預先 anchor 了人的判斷
- scanner 若做成 standing daemon / 跨 N repo 監看，會從「索引層」膨脹成 infra
- `candidate_registry` 若 commit 進 repo，可能變成新的 governance surface（要避免的東西）
- notify 語意若從「可以 review」漂成「已成熟/已升級」，就滑回 Promotion 自動化
...
| # | Question | 傾向 | 處置 |
|---|---|---|---|
| Q1 | candidate 要不要輸出 per-plan **confidence 數字**（如 `governance-pattern (0.81)`）？數字本身是機器對「屬於哪個 plan」的判定，可能違反 economics「只記實例不記結論」並 anchor 人的判斷。 | maintainer 對話傾向保留數字；我（設計者）傾向 v0 只列「命中哪條 criterion」平面指標，數字等 Phase 2 有誤報率數據再加。 | **still-open** — Phase 1 設計前須拍板 |
| Q2 | `candidate_registry` 用**單一 committed 檔**（降 churn、可跨 session 累積）還是 local-gitignored（不污染 repo，但存不住 ≥20）？ | 傾向單一 committed 檔 + 開頭明標 observation-only + economics 三條規則。 | **still-open** |
| Q3 | scanner 觸發模型：**agent-invoked**（手動掃當前 session）還是 task-completion hook（自動）？ | 傾向先 agent-invoked，避免膨脹成 infra；criteria 穩了再評估 hook。 | proposed → Phase 1 確認 |
...
- [ ] Q1–Q4 拍板（confidence 數字 / store 形態 / scanner 觸發 / evidence-rule 前置）
- [ ] 三個 plan 各長出 machine-readable `evidence-rule`（Phase 1 第一 artifact）
- [ ] `candidate_registry` 單檔建立並標明 observation-only + 三條規則
- [ ] agent-invoked scanner v0 可掃當前 session diff 比對 criteria 產生 candidate
- [ ] 累積觀察期後評估 §ADR Promotion Criteria（≥20 / >50% accept）→ 決定是否進 Phase 2
...
|---|---|---|
| Q1 confidence 數字 | still-open | 需 maintainer 拍板；設計者與對話傾向不同 |
| Q2 store 形態 | still-open | 與 ≥20 累積目標 / repo churn 衝突待解 |
| Q3 scanner 觸發 | proposed | 傾向 agent-invoked |
...
- [ ] Confirm source-of-truth：plan 各自擁有 evidence-rule / acceptance-gate / notify-rule；System 不寫死判準。
...
- [ ] 拍板 Q1（confidence 數字）/ Q2（store 形態）/ Q3（scanner 觸發）
- [ ] 三個 plan 各加 machine-readable `evidence-rule`（collect / 命中 criterion 定義）
- [ ] 建 `candidate_registry` 單檔（observation-only header + 三條規則）
- [ ] candidate schema：`id` / `source{repo,artifact,commit}` / `matched_plans[]` / `status{accepted}`
- [ ] agent-invoked scanner v0：掃當前 session diff/artifact → 比對 criteria → 產生 candidate（不自動寫入 plan）
- [ ] 人工 Accept 流程：accept → 寫入該 plan evidence；reject → 丟棄；defer → 留待
