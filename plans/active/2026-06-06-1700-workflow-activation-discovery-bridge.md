...
#### Phase B.1 — Evidence authority spike（reframed 2026-06-15）

> **Reframe**：不是「hook vs runtime」實作偏好之爭，而是一個 authority-boundary 問題——
> **「誰有資格知道真正發生過哪些 Read？」**。這繼承 §Evolution Ladder 的 Standing 原則
> （observation 可提議、不可鎖定）：evidence 該掛在能權威回答「哪些 Read 實際發生」的那一層。
> **定位**：小 spike，**不是**下一個主計畫；產出限縮成下方 5 題 + 一個 walkthrough，**禁止**
> 輸出 API / schema / implementation（那是 spike 通過後的事）。

**唯一要回答的問題**：Discovery evidence 應該掛在哪個 authority 邊界？

兩個候選邊界用同一組 acceptance 回答（不是比優點，是比 authority）：

| Acceptance 問題 | Hook（PostToolUse:Read 訂閱） | Runtime（file-read 觀測層） |
|---|---|---|
| 能看到實際發生的 Read | △ 依 hook delivery 時序 | ✅ |
| 能 append evidence | △ | ✅ |
| 能 re-score | △ | ✅ |
| 能避免 first-read 偏誤（reject Disc-2） | △ | ✅ |
| 能保留 replay | ○ | ✅ |
| 遞迴污染風險 | 高 | 中 |

**Spike 起始假設（偏 runtime，待驗，非定論）**：證據偏向 runtime——hook 路徑容易形成
控制層遞迴（`need discovery → 要求 Read → 重跑 discovery → need discovery → …`），把 advisory
層污染成會主動「要求」Read 的東西，違反「不新增任何 Read」。runtime 觀測路徑是
`Read 已發生 → PostToolUse(Read) → extract → append → re-score → telemetry →（可能 advisory）`，
天然只吸「已發生的 Read」，與 §Evolution Ladder 的 Acceptance 一致。**spike 仍須證實此假設，不得直接當結論。**

**Spike 必答 5 題**（產出僅此，連同 walkthrough；無 API/schema/code）：
- [ ] **Evidence source 是什麼**（吸哪一層的 Read 內容，邊界在哪）
- [ ] **誰觸發 re-score**（Read 事件 → re-score 的 owner）
- [ ] **Score lifetime**（per-task / TTL / 何時失效）
- [ ] **Advisory boundary**（達 threshold 後注入點；維持 active_route 不變）
- [ ] **Replay 怎麼做**（signal_snapshot 跨 scoring_version 重算）

**完成標準**：能用一個 walkthrough 回答即收——
`下関.md → 第一次 Read → travel score 0.22→0.71 → 是否 advisory？`（含「在哪一層、由誰、何時」）。
不要把 spike 做成整個 Piggyback Discovery 實作。
