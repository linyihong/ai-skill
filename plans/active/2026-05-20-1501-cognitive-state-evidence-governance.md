...
- 定義 cognitive cost governance，避免治理成本大於任務複雜度。
...
| Intent stability、claim scope、contradiction propagation、cognitive cost 的治理 gate | `governance/ai-runtime-governance/cognitive-state-governance.md` | 不取代 workflow success criteria。 |
| Evidence qualification、low-quality override 禁止規則 | `enforcement/evidence-hierarchy.md` | 不放 domain-specific reload set。 |
| Confidence decay、autonomy mode、recovery budget、human alignment trigger | `runtime/guards/` 或 `runtime/compiler/embedded_data.rb` | 若要 machine enforcement，必須確認 compiler 會讀到 source。 |
...
### 5.5 Confidence Decay and Integrity
...
### 5.7 Execution Intent Stability
...
### 5.11 Human Alignment Checkpoint
...
### 5.12 Cognitive Contamination Policy
...
### 5.13 Belief Garbage Collection
...
### 5.14 Cognitive Cost Governance

這一層的最大風險是 cognitive runtime inflation：治理成本高於任務本身，導致 agent 安全但不可用。
...
### 5.15 Source Freshness and Validity Governance
...
4. **與 Runtime Guard 的邊界**：`runtime/guards/circuit-breaker.yaml` 已有 `hallucination_risk` 與 `mismatch_escalation`。`confidence_decay` 若新增，應避免只是把 `mismatch_escalation` 換名字；它的獨立價值是「尚未 failure 但 autonomy 應下降」。`autonomy_modes` 可作為 guard action target，而不是新的哲學層。
...
- [ ] 確認 cognitive cost governance，避免小任務觸發過重治理。
...
- [ ] 定義 assumption lifecycle、assumption ledger、evidence lineage、contradiction propagation、evidence qualification、confidence integrity、claim scope、intent stability、cognitive contamination、belief GC、cognitive cost 的治理 gate。
...
- [ ] 將 evidence qualification、confidence integrity、intent drift、cognitive contamination 接到 escalation trigger。
...
- [ ] 定義 autonomy modes、recovery budget、recovery exit criteria、alignment threshold 與 contamination boundary trigger。
...
- Cognitive cost governance 會防止小任務啟動過重治理鏈。
- stale source-of-truth 不會被無條件當成 P0 evidence。
- 至少八個 failure-derived scenario 驗證 assumption drift、evidence qualification override、confidence integrity、claim scope overcompression、intent drift、contradiction propagation、recovery exit、cognitive contamination。
