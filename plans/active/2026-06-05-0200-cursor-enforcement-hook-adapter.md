...
### ⚠️ 連帶發現（高優先，需 Phase 0 釐清）

我們的 Go hook block 時回 `ExitValidationFailed = 30`，**不是 2**。

- **Cursor**：exit 2 才擋；exit 30 = 「其他 code」→ **fail-open 放行**。所以
  adapter 不能直接沿用 30，必須回 `exit 2` 或 `{"permission":"deny"}`。
- **Claude Code（既有！）**：Claude Code PreToolUse 文件亦為「exit 2 才 block，
  其他非零=非阻擋錯誤」。若屬實，**目前 Claude 端的 bootstrap gate 與 Phase 5
  gate 以 exit 30 回傳，可能根本沒有真正 block**（只是顯示 stderr 後仍放行）。
  本 plan 的 e2e 測試只驗了 CLI 自身 exit code，**未驗 Claude Code 是否真的
  攔下工具**。Phase 0 必須對真實 Claude Code 實測確認；若確認 leak，這是一個
  獨立的 high-severity 修正（所有 PreToolUse block 應回 exit 2）。
...
- [ ] **實測 Claude Code 是否真的因 `exit 30` 而 block**（建構一個會觸發
      bootstrap/workflow gate 的真實 session，看工具是否被擋）。
  - 若 Claude 在 exit 30 下**不擋** → 確立既有 leak，Phase 2 一併修成 exit 2。
  - 若 Claude 在 exit 30 下**會擋** → 記錄 Claude 對非零 code 的實際語意。
...
- [ ] 依 caller 輸出正確 block/allow：
  - Cursor：`exit 2` 或 stdout `{"permission":"deny","user_message","agent_message"}`（依 Phase 0 實測選定）。
  - Claude：依 Phase 0 真相 —— 若確立 exit 2 才 block，**把 PreToolUse block 路徑由 `ExitValidationFailed(30)` 改回 `2`**（新增 `ExitBlockToolUse = 2` 或重用 `ExitInvalidUsage` 不妥 → 用語意明確的常數）。
- [ ] 確保 allow 路徑 Cursor 回 `{"permission":"allow"}` / exit 0。
- [ ] 回歸：確認改 exit code 不破壞既有 commit-msg / 其他 hook（它們不經此路徑）。
...
| Q2 | Claude Code 在 `exit 30` 下到底擋不擋？（影響既有 enforcement 是否其實 leak） | Phase 0 對真實 Claude 實測 — **最高優先** |
