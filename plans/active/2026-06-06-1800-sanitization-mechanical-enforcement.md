...
- [ ] **Finding A 落地（Failure Authority Executor #1）**：`compileProjectMetadataDerived` 改 call `ClassifyFailureAuthority(metadata-file subject)` —— authoritative → hard-fail、non-authoritative（`shared_layer:false` / `.agent-goals/`）→ warn-only。**依賴**：先有 Authority Classification Contract 的 classifier 實作（step 3）。落地同時成為 family 第 4 個 sample，推進 N≥5 gate。
