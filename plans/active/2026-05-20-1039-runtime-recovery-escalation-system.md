...
- [x] Runtime source-of-truth 路徑被確認。
- [x] Plan 後續 phase 的檔案目標不再依賴不存在目錄。

#### Phase 0 Audit Result

`runtime/README.md` 仍列出多個 legacy YAML 路徑，但實際 workspace 中下列目錄目前不存在：`runtime/discovery/`、`runtime/recovery/`、`runtime/phases/`、`runtime/gates/`、`runtime/obligations/`、`runtime/transactions/`、`runtime/scheduler/`。對應資料已 embedded 在 `runtime/compiler/embedded_data.rb`，並由 compiler 寫入 `runtime/runtime.db`。

| Runtime domain | Standalone YAML exists? | Current source-of-truth | Runtime DB signal |
| --- | --- | --- | --- |
| Discovery checkpoints | no | `runtime/compiler/embedded_data.rb` (`DISCOVERY_CAPABILITY_CHECKPOINTS`) | `discovery_checkpoints = 4` |
| Phase machine | no | `runtime/compiler/embedded_data.rb` (`PHASES_PHASE_MACHINE`) | `phases = 9` |
...
- `runtime/compiler/embedded_data.rb` (embedded recovery source)
- `runtime/compiler/compiler-engine.rb` (only if new tables or compile targets are needed)
- `runtime/runtime.db`
- `runtime/README.md`
