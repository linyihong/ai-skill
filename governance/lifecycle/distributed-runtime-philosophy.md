...
1. **Transaction Runtime 為基礎**：distributed transaction 共用 `runtime/runtime.db` 的 `transaction_states` / `transaction_transitions` state machine（source：`runtime/compiler/embedded_data.rb`），擴充 distributed lock 與 lease 機制。
2. **Phase Reconciliation 為核心**：多 agent 間的 phase inconsistency 透過 `runtime/runtime.db` 的 `phase_reconciliation` / `recovery_strategies`（source：`runtime/compiler/embedded_data.rb`）解決。
...
- [`runtime/compiler/embedded_data.rb`](../../runtime/compiler/embedded_data.rb) — transaction / recovery state machine 的 source
