...
1. **Transaction Runtime 為基礎**：distributed transaction 共用 `runtime/runtime.db` 的 `transaction_states` / `transaction_transitions` state machine（source：`runtime/compiler/compiler-rules.yaml`），擴充 distributed lock 與 lease 機制。
2. **Phase Reconciliation 為核心**：多 agent 間的 phase inconsistency 透過 `runtime/runtime.db` 的 `phase_reconciliation` / `recovery_strategies`（source：`runtime/compiler/compiler-rules.yaml`）解決。
...
- [`runtime/compiler/compiler-rules.yaml`](../../runtime/compiler/compiler-rules.yaml) — transaction / recovery state machine 的 source
