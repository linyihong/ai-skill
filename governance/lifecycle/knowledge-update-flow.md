...
Step 3: 決定 Promotion Target ─ 應該放到哪一層？
    ...
    ▼
Step 3a: 確認目標文件語言 ── 【強制】寫入前先檢查目標文件的語言規範
    │   ⛔ 未確認目標語言與保留原詞理由，不得寫入可重用文件
    │                           (neutral-language.md + language-policy.yaml)
    ▼
...
- 未讀取目標文件並判斷正文主要語言前，不得寫入可重用文件。
- 未確認外語片段是否屬於必要專有名詞、路徑、命令、API 欄位、工具名、搜尋關鍵字、地名或原文引用前，不得保留整句外語正文。
- 寫入後若 diff 出現整句外語正文、混語段落或表格欄位語言不一致，不得進入 commit；必須先修正或明確記錄保留理由。
- 對 `enforcement/`、`workflow/`、`analysis/`、`intelligence/`、`feedback/`、`knowledge/`、`memory/`、`models/`、`governance/` 等本庫可重用文件，預設正文語言為繁體中文。
...
4. **記錄保留外語的理由**：
   - 可保留：路徑、命令、API 欄位、程式符號、工具名、固定 protocol 名稱、搜尋關鍵字、地名、原始 UI / 法規 / 介面詞。
   - 不可保留：可翻譯的一般說明句、整段外語正文、混用語言的 checklist 或表格欄位。

5. **寫入後驗證**：
   - diff review 必須檢查新增行是否符合目標文件語言。
   - 若保留外語，final response 或 commit message 必須簡述保留理由。
   - 若本步驟未完成，不得宣告知識更新完成。
...
□ Step 3a: [強制] 確認目標文件語言（neutral-language.md + language-policy.yaml）
           ⛔ 未確認目標語言、外語保留理由與寫入後語言一致性，不得寫入/commit
