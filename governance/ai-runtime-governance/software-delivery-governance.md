...
本文件把 docs-first BDD closure、contract-first development 與 development guidance 的風險翻譯方法轉譯成 AI runtime software-delivery governance。原始 intelligence 回答「為什麼 observable behavior 變更前要更新 owning contract」；本文件定義 change intake、contract precedence、BDD closure、artifact completeness、performance evidence 與 same-session documentation closure 的治理 gate。
