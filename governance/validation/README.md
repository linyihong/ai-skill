...
| SQLite runtime index boundary | 執行 `ruby scripts/validate-runtime-sqlite-index.rb`，確認 SQLite / FTS 只作 generated lookup cache，DB 可重建、被 git ignore，且不取代 canonical source。 | SQLite generator、query helper、feedback lesson index 或 runtime lookup cache 變更。 |
