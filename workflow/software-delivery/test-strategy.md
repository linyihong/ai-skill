...
| User-visible KPI / counter labels | 防止 DB 或 API 已正確但使用者仍看到錯誤渲染標籤（同 entity、同 path） | 分層證明：durable state → API / listing SQL（產品公式）→ browser DOM（同 entity id + 使用者路徑）。若 layer N 通過但未驗 layer N+1，先分類 projection / client cache，再決定是否改 SQL。見 [`validation.md`](validation.md) § User-Visible Counter Depth Gate |
| L2 contract / BDD vs L3 browser UX | 防止 Gherkin / source assert 通過但實際 browser 行為仍壞 | 補 L3 integration + evidence envelope（`evidence:user_visible`、`evidence:temporal_behavior` 等）；gate `requires:` 只列 evidence_type。見 [`validation/failure-evolution-catalog.md`](validation/failure-evolution-catalog.md) |
...
| 使用者可見 KPI / 計數標籤是否被質疑？ | 同一 entity id、同一路徑上跑 durable state → API/listing SQL → browser DOM；勿在 API 已對時只改 SQL。見 [`validation.md`](validation.md) § User-Visible Counter Depth Gate |
| L2 BDD / contract 通過但使用者仍回報 UX 壞？ | 視為 projection break；補 L3 browser integration + evidence envelope，勿只加 BDD string assert |
...
| RWD / 視覺縮放是否被使用者、render context 或裝置 emulation 反證？ | 不只檢查 CSS 或 BDD marker；用 headless browser / WebDriver / CDP 設定代表性 render contexts（依 [`render-contexts`](../../intelligence/engineering/render-contexts/README.md)），至少包含 desktop + mobile 或同等 wide / narrow context。若問題涉及 resize、reload、rotate 或 restore，依 [`state-transition-validation`](../../intelligence/engineering/execution/validation-reasoning/state-transition-validation.md) 加 transition sequence。量 `window.innerWidth`、`document/body.scrollWidth`、app shell、fixed navigation 與主要 scroll content，並列出排除 intended horizontal scroller 後的 overflow offenders |
