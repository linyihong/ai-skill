...
| RWD / browser layout | 防止 CSS marker 通過但瀏覽器實際 viewport 仍 overflow | 使用真實 browser engine 的 integration test 量 `document`、`body`、app shell、fixed navigation 與主要 scroll content 的 computed width；區分 intended horizontal scroller 與非預期 overflow |
...
| RWD / 視覺縮放是否被使用者或裝置 emulation 反證？ | 不只檢查 CSS 或 BDD marker；用 headless browser / WebDriver / CDP 設定代表性 viewport，量 `window.innerWidth`、`document/body.scrollWidth`、app shell、fixed navigation 與主要 scroll content，並列出排除 intended horizontal scroller 後的 overflow offenders |
