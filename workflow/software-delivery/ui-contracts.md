...
|---|---|
| `id` | `sd-ui-contracts` |
| `purpose` | 在前端、行動、CLI、SDK 或其他 consumer 實作前，定義 screen mapping、consumer 需要的資料、screen/state 行為、render context、UI interaction、view model derivation、accessibility expectations 與 screen traceability |
| `type` | `execution` |
| `tags` | artifact-gate, contract, frontend, consumer, ui |
| `load_when` | 前端 / 行動 / CLI / SDK / consumer surface 需要和 provider 平行實作，或 AI agent 要從需求生成 UI、state、render contexts、hooks、store、tests |
...
| Render Context Coverage | desktop、tablet、mobile、narrow_mobile、orientation、safe area、input modality 等 presentation constraints |
...
| `entrypoints` | 使用者可從哪些 route、screen、deep link、modal、drawer、notification、external link 或 system callback 進入此 screen / flow |
| `navigation_contract` | 進入、切換、返回、replace vs push、redirect、fallback、history ownership 與 cross-screen navigation expectation |
...
screens:
  ...
  - player
entrypoints:
  - home ranking card -> player
  - player episode sheet -> detail
navigation_contract:
  forward: external content entries open player directly
  back: detail opened from player returns to player; direct detail deep link falls back to home
apis:
  - GET /videos
  - GET /episodes
  - POST /watch-history
...
| `given_state` | 初始資料、權限、session、feature flag、render_context 條件 |
| `action` | 使用者或系統觸發的操作 |
...
| `navigation` | route、modal、drawer、back behavior、deep link、redirect、history stack、router push/replace/back、fallback destination、cross-entry return path |
...
| `render_contexts` | 此 screen 必須維持合約的 presentation contexts，例如 desktop、tablet、mobile、narrow_mobile、landscape、safe_area、touch_only、keyboard_only |
...
`render_contexts` 不應列成工具或裝置 SKU（例如特定手機型號）。它描述同一個 UI contract 必須成立的 presentation context：可用寬度、orientation、safe area、input modality、zoom 或 assistive mode。工具設定（Playwright project、browser device emulation、real device farm）只是取得 evidence 的方式。
...
- 當同一 screen 可由多個入口打開，或 route 之間存在 push/replace/back/deep-link fallback 差異時，Screen Mapping 必須列出 `entrypoints`、`navigation_contract` 與返回棧 owner；不能只列受影響 screens。
...
- Render context 不應綁死特定 device name；以 desktop、tablet、mobile、narrow_mobile、orientation、safe area、input modality 或 zoom 條件描述 contract surface。
...
- 導航與返回棧 trace 必須能追到 executable interaction proof，例如 browser integration、E2E、runtime trace，或明確標記 `manual-evidence` / `pending-runner`；static marker 只能證明 contract 存在，不能單獨證明瀏覽器歷史行為。
...
- Browser integration / E2E navigation test，證明 history stack、router push/replace/back、deep-link fallback 與跨入口返回路徑在真實瀏覽器成立。
- Responsive / render-context validation，證明同一個 Screen Contract 在至少 desktop 與 mobile context 下仍符合 layout、interaction、focus、安全區域與 touch target 期望。
