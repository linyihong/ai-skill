...
> | UI governance（UI compliance / design system / evidence） | **已抽出** → [`ui-governance.md`](ui-governance.md)（含 governance domain、validation mechanism、evidence class、severity policy、project-local design-system boundary） | `sd-ui-governance` | UI / consumer surface 需要 design-system enforcement、accessibility evidence、behavior pattern checks、visual baseline review、AI visual review scoping 或 UI compliance completion claim |
